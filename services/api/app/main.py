...
async def _lifespan(app: FastAPI):
    ...
    # OCR prewarm (below) shares the same heavy-startup path so a deferred
    # first boot runs seed → prewarm sequentially in one background thread.
    def _seed_demo_inspections() -> None:
        if not settings.seed_demo_inspections:
            return
    ...
    def _prewarm_ocr() -> None:
        ...
        if not (
            settings.perception_ocr_prewarm
            and settings.perception_ocr_backend == "paddle"
        ):
            return
    ...
    if settings.defer_heavy_startup:
        # Deployment (see docs/deployment.md): bind the port and pass the
        # platform health check first; the OCR work finishes in the background
        # (idempotent — a restart simply resumes).
        threading.Thread(
            target=_heavy_startup, name="heavy-startup", daemon=True
        ).start()
    else:
        _heavy_startup()
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    # Single-origin deployment: the SPA index route is registered BEFORE the
    # meta root below so it takes precedence at "/" when enabled.
    _mount_static_site(app, settings)
...
def _mount_static_site(app: FastAPI, settings: Settings) -> None:
    """Serve the built web client from the API origin (deployment only).

    Enabled by ``STATIC_DIST_DIR`` pointing at a vite build output. API routes
    (registered earlier) keep precedence; ``/assets`` is served by StaticFiles
    and a catch-all serves any other real file, falling back to ``index.html``
    for client-side routes. Unknown ``api/`` paths keep JSON 404 semantics.
    Unset (local dev) → no-op; Vite serves the client.
    """
    if not settings.static_dist_dir:
        return
    dist = Path(settings.static_dist_dir)
    index = dist / "index.html"
    if not index.is_file():
        logger.warning("static_dist_missing", path=str(dist))
        return
    ...
    def spa(full_path: str) -> FileResponse:
        if full_path.startswith("api/"):
            # Unknown API path: structured 404, never the SPA shell.
            raise NotFoundError("Unknown API path.")
